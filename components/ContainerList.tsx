  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
           <h1 className="text-2xl font-bold text-slate-800">Tracked Containers</h1>
           <p className="text-slate-500 text-sm mt-1">Manage active pharmaceutical shipments</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {containers.map(container => (
          <Link key={container.id} to={`/containers/${container.id}`} className="group relative block">
            ...
            <div className="relative bg-white p-5 rounded-xl border border-slate-200 shadow-sm h-full flex flex-col justify-between group-hover:-translate-y-1 group-hover:-translate-x-1 transition-transform">
               ...
               <div className="flex justify-between items-start mb-4">
                 <div className="flex items-center space-x-3">
                   ...
                   <div>
                     <h3 className="font-bold text-slate-800 text-lg leading-tight">{container.id}</h3>
                     <p className="text-xs text-slate-500">{container.drugName}</p>
                   </div>
                 </div>
