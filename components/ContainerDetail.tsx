...
const ContainerDetail: React.FC = () => {
  ...
  if (!container) {
    return <div className="p-8 text-center">Container not found</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center space-x-4 mb-6">
        <Link to="/containers" className="p-2 bg-white rounded-full border border-slate-200 hover:bg-slate-50 text-slate-500 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center">
            {container.name} 
            <span className="ml-3 text-sm font-normal text-slate-500 bg-slate-100 px-2 py-1 rounded border border-slate-200">
              {container.id}
            </span>
          </h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
             <div className="flex items-center justify-between mb-6">
               <h3 className="font-semibold text-slate-700">Live Status</h3>
             ...
             <div className="grid grid-cols-2 gap-4">
               <div className="text-center p-4 rounded-xl bg-slate-50 border border-slate-100">
                 <Thermometer className={`mx-auto mb-2 ${container.currentReading.temperature > settings.tempMax ? 'text-red-500' : 'text-slate-400'}`} />
                 <div className="text-3xl font-bold text-slate-800">{container.currentReading.temperature}°C</div>
                 <div className="text-xs text-slate-400 mt-1">Temperature</div>
               </div>
               <div className="text-center p-4 rounded-xl bg-slate-50 border border-slate-100">
                 <Droplets className="mx-auto mb-2 text-blue-400" />
                 <div className="text-3xl font-bold text-slate-800">{container.currentReading.humidity}%</div>
                 <div className="text-xs text-slate-400 mt-1">Humidity</div>
               </div>
             </div>

             {container.isCoolingActive && (
               <div className="mt-4 bg-blue-50 border border-blue-100 p-4 rounded-lg flex items-center text-blue-700 text-sm">
                 <Zap size={18} className="mr-2 animate-bounce" />
                 <div>
                   <p className="font-bold">Corrective Action Active</p>
                   <p className="text-xs text-blue-600">Cooling system engaged to lower temp.</p>
                 </div>
               </div>
             )}
          </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  />
                  <span className="w-16 text-center font-mono font-bold text-purple-700 bg-purple-50 px-2 py-1 rounded">
              ...
              <div className="pt-4 border-t border-slate-100">
                 ...
                 <button 
                   ...
                   className="w-full flex items-center justify-center space-x-2 py-2 bg-orange-50 hover:bg-orange-100 text-orange-600 border border-orange-200 rounded-lg transition-colors"
                 >
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
          <h3 className="font-semibold text-slate-700 mb-6">Real-time Telemetry</h3>
          <div className="flex-1 min-h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis dataKey="time" stroke="#94a3b8" fontSize={12} tick={{dy: 10}} />
